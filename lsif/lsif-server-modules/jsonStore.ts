...
export class JsonStore extends Database {
  ...
  private doProcessDocument(document: Document): void {
    const contents = document.contents === undefined ? "No content provided." : document.contents;
  ...
  private item(
    ...
    if (value.label === "referenceResult") {
    ...
    for (const document of value.documents) {
      ...
      for (const item of contains) {
        ...
        if (JsonStore.containsPosition(item, position)) {
          if (!candidate) {
            candidate = item;
          } else if (JsonStore.containsRange(candidate, item)) {
            candidate = item;
          }
        }
      }
